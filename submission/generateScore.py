...
# Builds the color list from the label image
for line in fin:
	tmp = line.strip('\n').split(" ")
	key_labels.append((int(tmp[0]),int(tmp[1]),int(tmp[2])))
	key_assign[(int(tmp[0]),int(tmp[1]),int(tmp[2]))] = (0,0,0)
	
#Builds a list of colors from the generated list of colors. 
...
output1 = ''
output2 = ''
#loop through a directory and process each image
for dirname,subdir,filelist in os.walk(sys.argv[2]):
	for fname in sorted(filelist):
		...
		for i in range(int(sys.argv[3])):
			...
			# This function processes each image and corrects the jpg error. This will do so for every point in a given object region
			...
			check = 0
			# Assign values for what class belongs to a region
			for val in key_assign.keys():
		...
		# This block is for batch running and writing to csv files.
